...
const maxSchedules = process.env.MAX_NUMBER_SCHEDULE;
    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.arrSchedule || !data.doctorId || !data.date) {
                ...
                resolve({
                    errCode: 0,
                    errMessage: `Create doctor's schedule success!`
                })
            }
...
let getExtraProfileDoctorById = (inputId) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!inputId) {
                resolve({
